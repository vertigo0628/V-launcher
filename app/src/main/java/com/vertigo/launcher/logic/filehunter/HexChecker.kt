...
object HexChecker {
     ...
     * via Shizuku's shell (accessed through [FileHunterShizukuHelper.executeShizukuCommand]).
    ...
    private fun isJpegViaShizuku(path: String): Boolean {
        return try {
            ...
            val output = FileHunterShizukuHelper.executeShizukuCommand(command).trim()
